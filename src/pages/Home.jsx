  ...
  return (
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-navy-950">
      ...
      <div className="absolute inset-0 z-[3] pointer-events-none opacity-[0.03]"
        style={{ backgroundImage: 'linear-gradient(rgba(220,38,38,0.3) 1px, transparent 1px), linear-gradient(90deg, rgba(220,38,38,0.3) 1px, transparent 1px)', backgroundSize: '80px 80px' }} />
      ...
      <div className="absolute right-5 sm:right-8 top-1/2 -translate-y-1/2 z-30 flex flex-col gap-3">
        {heroImages.map((_, i) => (
          <button key={i} onClick={() => setCurrentSlide(i)}
            className={`w-[3px] transition-all duration-700 ${i === currentSlide ? 'h-10 bg-red-500' : 'h-4 bg-white/20 hover:bg-white/40'}`} aria-label={`Slide ${i+1}`} />
        ))}
      </div>

      <NoiseTexture opacity={0.03} />
      <div className="absolute top-[15%] left-0 w-[2px] h-32 sm:h-48 bg-gradient-to-b from-transparent via-red-500 to-transparent z-20" />
      ...
      <motion.div className="relative z-20 flex flex-col justify-center h-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ y: textY, opacity }}>
        <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 1, delay: 0.3 }}
          className="w-16 h-[2px] bg-gradient-to-r from-red-500 to-red-400/50 mb-6 origin-left" />

        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4 }}
          className="text-red-400 text-xs sm:text-sm font-semibold uppercase tracking-[0.3em] mb-8" style={{ fontFamily: 'var(--font-sans)' }}>
          {hero.badge}
        </motion.p>
        ...
        <div className="overflow-hidden">
          {(hero.titleParts || []).map((part, i) => (
            <motion.div key={i} initial={{ y: '110%' }} animate={{ y: 0 }}
              ...
              <h1 className={`font-heading leading-[0.92] tracking-tight ${part.highlight ? 'bg-gradient-to-r from-red-500 via-red-400 to-red-600 bg-clip-text text-transparent' : 'text-white'}`}
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 1 }}
          className="flex items-center gap-3 mt-8 bg-red-500/10 border border-red-500/20 px-4 py-2.5 w-fit">
          <div className="relative">
            <div className="w-2.5 h-2.5 bg-red-500 rounded-full" />
            <motion.div animate={{ scale: [1, 2, 1], opacity: [0.5, 0, 0.5] }} transition={{ repeat: Infinity, duration: 2 }}
              className="absolute inset-0 w-2.5 h-2.5 bg-red-500 rounded-full" />
          </div>
          <span className="text-red-400/80 text-xs uppercase tracking-[0.2em] font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>24/7 Active Monitoring</span>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 1.4 }}
          ...
          <Link to="/contact" className="group inline-flex items-center gap-3 bg-red-600 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-red-500 hover:shadow-xl hover:shadow-red-500/20 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaPrimary} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
          </Link>
          <Link to="/projects" className="group inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:border-red-500/50 hover:text-red-400 hover:bg-white/5 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
            {hero.ctaSecondary}
          </Link>
        </motion.div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }}
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 2 }}>
          <ArrowDown size={14} className="text-red-500/40" />
        </motion.div>
      </motion.div>
...
function EmergencyStrip() {
  ...
  return (
    <section className="relative bg-red-600 py-4 overflow-hidden z-10">
      ...
      <div className="relative z-20 max-w-7xl mx-auto px-5 sm:px-8 flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-white text-xs sm:text-sm font-bold uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Emergency Response 24/7</span>
        </div>
...
function MarqueeTicker() {
  const items = ['CCTV INSTALLATION', 'ACCESS CONTROL', 'ALARM SYSTEMS', 'ELECTRIC FENCING', 'GATE AUTOMATION', 'FIRE DETECTION', '24/7 MONITORING'];
  const repeated = [...items, ...items, ...items, ...items];
  return (
    <section className="bg-navy-950 border-y border-red-500/10 py-5 sm:py-6 overflow-hidden">
      <div className="animate-marquee flex whitespace-nowrap">
        {repeated.map((item, i) => (
          <span key={i} className="flex items-center gap-6 sm:gap-8 mx-6 sm:mx-8">
            <span className="text-red-500/50 font-heading text-lg sm:text-2xl tracking-wider">{item}</span>
            <span className="text-red-500/15 text-sm">&diams;</span>
          </span>
        ))}
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="mb-14 sm:mb-20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <p className="text-red-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Security Solutions</p>
              <h2 className="font-heading text-white leading-[0.92]" style={{ fontSize: 'clamp(2rem, 4.5vw, 3.5rem)' }}>
                Complete <span className="text-red-500">Protection</span>
              </h2>
            </div>
            <Link to="/services" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-red-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            return (
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                ...
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`}
                  ...
                  {/* Camera corners */}
                  <div className="absolute top-3 left-3 w-6 h-6 border-t-2 border-l-2 border-red-500/40 z-10" />
                  <div className="absolute top-3 right-3 w-6 h-6 border-t-2 border-r-2 border-red-500/40 z-10" />
                  <div className="absolute bottom-3 left-3 w-6 h-6 border-b-2 border-l-2 border-red-500/40 z-10" />
                  <div className="absolute bottom-3 right-3 w-6 h-6 border-b-2 border-r-2 border-red-500/40 z-10" />

                  {i === 0 && (
                    <div className="absolute top-5 right-8 z-10 flex items-center gap-2">
                      <motion.div animate={{ opacity: [1, 0, 1] }} transition={{ repeat: Infinity, duration: 1.5 }} className="w-2 h-2 bg-red-500 rounded-full" />
                      <span className="text-red-500 text-[10px] font-mono uppercase tracking-wider">REC</span>
                    </div>
                  )}

                  <div className="absolute top-5 left-5 z-10 w-10 h-10 bg-red-600 flex items-center justify-center group-hover:bg-red-500 transition-colors duration-500">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 z-10">
                    ...
                    <div className="flex items-center gap-2 mt-3 text-red-500 group-hover:translate-x-1 transition-transform duration-300">
                      <span className="text-xs uppercase tracking-[0.2em]" style={{ fontFamily: 'var(--font-sans)' }}>Explore</span>
                      <ArrowRight size={14} />
                    </div>
                  </div>
                  <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-red-500 to-red-400 z-10" />
                </Link>
              </motion.div>
            );
          })}
    ...
    <section ref={containerRef} className="bg-navy-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 mb-12 sm:mb-16">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }}
          ...
          <div>
            <div className="w-12 h-[2px] bg-red-500 mb-6" />
            <p className="text-red-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>Installations</p>
            <h2 className="font-heading text-white leading-[0.92]" style={{ fontSize: 'clamp(2.5rem, 6vw, 5rem)' }}>Our <span className="text-red-500">Portfolio</span></h2>
          </div>
          <Link to="/projects" className="group text-white/30 text-xs uppercase tracking-[0.2em] flex items-center gap-2 hover:text-red-500 transition-colors" style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            <div key={project.slug} className="group relative flex-shrink-0 w-[300px] sm:w-[360px] lg:w-[420px] overflow-hidden">
              <div className="aspect-[3/4] overflow-hidden relative">
                ...
                <div className="absolute top-5 left-5 z-10">
                  <span className="bg-red-600 text-white text-[10px] uppercase tracking-[0.2em] px-3 py-1.5 font-semibold" style={{ fontFamily: 'var(--font-sans)' }}>{project.category}</span>
                </div>
              </div>
  ...
  return (
    <section ref={ref} className="relative bg-navy-950 overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-48 h-48 bg-red-500/5 rounded-full blur-3xl" />
        <div className="absolute top-1/2 right-1/4 -translate-y-1/2 w-48 h-48 bg-red-500/5 rounded-full blur-3xl" />
      </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-16">
          {stats.map((stat, i) => (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className="font-heading text-red-500 leading-none" style={{ fontSize: 'clamp(2.8rem, 6vw, 5rem)', textShadow: '0 0 40px rgba(220,38,38,0.15)' }}>
              ...
              {i < stats.length - 1 && <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 h-16 w-[1px] bg-gradient-to-b from-transparent via-red-500/15 to-transparent" />}
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            <div className="w-12 h-[2px] bg-red-500 mb-6" />
            <p className="text-red-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>About Us</p>
            <h2 className="font-heading text-white leading-[0.95] mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The {business.name}<br /><span className="text-red-500">Story</span>
            </h2>
            ...
            <div className="flex gap-10 sm:gap-16">
              {[{ val: business.established, lbl: 'Founded' }, { val: business.projectsCompleted, lbl: 'Installed' }, { val: business.employees, lbl: 'Technicians' }].map(s => (
                <div key={s.lbl}>
                  <div className="text-red-500 font-heading text-3xl sm:text-4xl leading-none">{s.val}</div>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.2 }} className="relative">
            <div className="relative">
              ...
              <div className="absolute -top-3 -left-3 w-16 h-16 border-t-2 border-l-2 border-red-500/30" />
            </div>
          </motion.div>
        </div>
      </div>
...
function WhyChooseUs() {
  ...
  const { business } = siteData;
  const points = [
    { title: 'Certified Technicians', desc: 'Factory-trained engineers with international certifications in CCTV, access control, and alarm system installation.' },
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }} className="relative">
            ...
            <div className="absolute -top-3 -left-3 w-20 h-20 border-t-2 border-l-2 border-red-500/40" />
            <div className="absolute -bottom-3 -right-3 w-20 h-20 border-b-2 border-r-2 border-red-500/40" />
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9, delay: 0.15 }}>
            <div className="w-12 h-[2px] bg-red-500 mb-6" />
            <p className="text-red-500/60 text-xs uppercase tracking-[0.3em] mb-3" style={{ fontFamily: 'var(--font-sans)' }}>The Difference</p>
            <h2 className="font-heading text-white leading-[0.95] mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-red-500">{business.name}</span>
            </h2>
            <div className="space-y-8">
              {points.map((point, i) => (
                <motion.div key={point.title} initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                  ...
                  <div className="shrink-0 mt-1"><div className="w-8 h-8 bg-red-600 flex items-center justify-center"><CheckCircle size={16} weight="fill" className="text-white" /></div></div>
                  <div>
    ...
    <section ref={ref} className="relative bg-navy-950 py-24 sm:py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center">
          <Quotes size={48} weight="fill" className="text-red-500/15 mx-auto mb-8" />
          <AnimatePresence mode="wait">
            <motion.div key={active} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.6 }}>
              ...
              <div className="flex flex-col items-center gap-3">
                {t.avatar && <img src={t.avatar} alt={t.name} className="w-12 h-12 rounded-full object-cover object-center border-2 border-red-500/30" loading="lazy" />}
                ...
                <div className="flex items-center gap-0.5 mt-1">{[...Array(t.rating)].map((_, i) => <Star key={i} size={12} weight="fill" className="text-red-500" />)}</div>
              </div>
            </motion.div>
          </AnimatePresence>
          <div className="flex items-center justify-center gap-6 mt-12">
            <button onClick={prev} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-red-500 hover:border-red-500/30 transition-colors" aria-label="Previous"><CaretLeft size={16} /></button>
            <div className="flex gap-2">{homeTestimonials.map((_, i) => (
              <button key={i} onClick={() => setActive(i)} className={`h-[2px] transition-all duration-500 ${i === active ? 'w-10 bg-red-500' : 'w-3 bg-white/10 hover:bg-white/25'}`} aria-label={`Testimonial ${i+1}`} />
            ))}</div>
            <button onClick={next} className="w-10 h-10 border border-white/10 flex items-center justify-center text-white/30 hover:text-red-500 hover:border-red-500/30 transition-colors" aria-label="Next"><CaretRight size={16} /></button>
          </div>
        </motion.div>
      </div>
    </section>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-5 sm:px-8 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.9 }}>
          ...
          <h2 className="font-heading text-white leading-[0.95] mb-6" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
            Protect What <span className="text-red-500">Matters Most</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/contact" className="group inline-flex items-center gap-3 bg-red-600 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-red-500 hover:shadow-xl hover:shadow-red-500/20 transition-all duration-500" style={{ fontFamily: 'var(--font-sans)' }}>
              Free Security Assessment <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </Link>
